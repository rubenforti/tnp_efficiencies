...
def fit_on_bin(type_eff, workspace, cond, bin, bkg_pdf, test_bkg=False, verb=-1):
    ...
    if type(workspace[f'PDF_{cond}_({bin[0]},{bin[1]})']) is ROOT.RooAddPdf:
        ...
        print("Not possible to refit an existing PDF! Returning the results obtained previously")
        ...
        if bkg_pdf == 'cmsshape':
            alpha = ROOT.RooRealVar(
                workspace["alpha"], f"alpha_{cond}_({bin[0]},{bin[1]})")
            beta = ROOT.RooRealVar(
                workspace["beta"], f"beta_{cond}_({bin[0]},{bin[1]})")
            gamma = ROOT.RooRealVar(
                workspace["gamma"], f"gamma_{cond}_({bin[0]},{bin[1]})")
            peak = ROOT.RooRealVar(
                workspace["peak"], f"peak_{cond}_({bin[0]},{bin[1]})")
...
def independent_efficiency(type_eff, bins, results, direct_mode=True,
    ...
    for idx in range(len(bins_list[0])):
        ...
        res_pass = fit_on_bin(type_eff, ws, 'pass', (bin_pt, bin_eta), 'expo',
                              test_bkg=test_bkg, verb=verbose)
        res_fail = fit_on_bin(type_eff, ws, 'fail', (bin_pt, bin_eta), 'expo',
                              test_bkg=test_bkg, verb=verbose)
...
if __name__ == '__main__':
    ...
    independent_efficiency(t, bins, results, direct_mode=True, verbose=-1)
    '''
    '''
